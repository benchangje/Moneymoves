...
export default function ReviewForm({ listing }) {
  const { user } = useAuth();
  ...
  const canReview = Boolean(user && listing?.id && listing?.ownerUid && user.uid !== listing.ownerUid);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!canReview) {
      setMessage('You need to sign in and choose a different user to review.');
      return;
    }
